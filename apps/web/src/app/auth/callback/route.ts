...
export async function GET(request: NextRequest) {
  ...
  if (oauthError) {
    return NextResponse.redirect(
      loginErrorUrl(request, "access_denied", nextPath),
    );
  }

  if (!code) {
    return NextResponse.redirect(
      loginErrorUrl(request, "missing_code", nextPath),
    );
  }
  ...
  if (error || !data.session) {
    return NextResponse.redirect(
      loginErrorUrl(request, "callback_failed", nextPath),
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      loginErrorUrl(request, "sync_failed", nextPath),
    );
  }
